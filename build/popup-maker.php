<?php
/**
 * Plugin Name:       Advanced Pop-up Maker
 * Plugin URI:        https://wordpresstelex.com/popup-maker
 * Description:       Create high-converting pop-ups with advanced targeting, exit-intent detection, and native Gutenberg integration. Performance-optimized solution for modern WordPress websites.
 * Version:           0.1.0
...
if ( ! function_exists( 'popup_maker_block_init' ) ) {
	 ...
	 */
	function popup_maker_block_init() {
		register_block_type( __DIR__ . '/build/' );
	}
}
add_action( 'init', 'popup_maker_block_init' );
...
if ( ! function_exists( 'popup_maker_enqueue_frontend_assets' ) ) {
	...
	function popup_maker_enqueue_frontend_assets() {
		if ( has_block( 'telex/block-popup-maker' ) ) {
			wp_enqueue_style(
				'popup-maker-frontend',
			...
			wp_localize_script( 'popup-maker-frontend', 'popupMakerAjax', array(
